...
func (c *criService) RunPodSandbox(ctx context.Context, r *runtime.RunPodSandboxRequest) (_ *runtime.RunPodSandboxResponse, retErr error) {
	...
	defer func() {
		if retErr != nil {
			...
			if _, cleanupErr = task.Delete(deferCtx, WithNRISandboxDelete(id), containerd.WithProcessKill); cleanupErr != nil && !errdefs.IsNotFound(cleanupErr) {
				log.G(ctx).WithError(cleanupErr).Errorf("Failed to delete sandbox container %q", id)
			}
		}
	}()
